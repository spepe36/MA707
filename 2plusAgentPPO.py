...
def train_ppo(num_episodes=100, rollout_steps=2000, gamma=0.99, lam=0.95, render_mode='False',
              policy_path=None, critic_path=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    env = NeuralDash(render_mode=render_mode, number_of_enemies=3)
    obs_dims = 9
    action_dims = 5
    ...
    for episode in range(num_episodes):
        ...
        if episode == num_episodes // 2:
            save_model(policy, critic, directory="models", episode=episode)
...
def save_model(policy, critic, directory="models", episode=None):
    # Create directory if it doesn't exist
    os.makedirs(directory, exist_ok=True)

    # Save policy and critic models
    policy_path = os.path.join(directory, f"policy{'_ep' + str(episode) if episode is not None else ''}multiagent.pth")
    critic_path = os.path.join(directory, f"critic{'_ep' + str(episode) if episode is not None else ''}multiagent.pth")
...
train_ppo(num_episodes=10000, rollout_steps=1500, gamma=0.8, lam=0.95)
